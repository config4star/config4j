...
match, say, the names of all operations within a specific class ("B.*"),
all create-style operations, regardless of the class in which they
appear ("*.create*"), or all operations in all classes ("*").
...
Examples of running the demo:
    ...
    runDemo.sh -h                             (prints a usage statement)
    runDemo.sh -cfg runtime.cfg -scope foo

In the above examples, replace "runDemo.sh" with "runDemo.bat" if you
are on Windows.
